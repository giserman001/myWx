				...
				<swiper :current="swiperCurrent" @transition="transition" @animationfinish="animationfinish" class="swiperBox">
					<swiper-item class="swiper-item" v-for="(item, index) in tabs" :key="index">
						<scroll-view scroll-y class="scroll-view" @scrolltolower="onreachBottom" @scrolltoupper="onreachTop">
							<u-card :show-head="false" padding="0" :foot-border-top="false" :head-border-bottom="false" :border="false" v-for="item in 10" :key="item">
							...
							<view @click="getMore(index)" class="showMore" style="">点击加载更多</view>
						</scroll-view>
					</swiper-item>
				</swiper>
			</view>
...
export default {
	...
	onShow() {
		this.City = this.$api.getSite('city') || '全国';
	},
	onLoad(data) {
		this.City = data.city || '全国';
		this.init();
	},
	components: {
		//引用mSearch组件，如不需要删除即可
